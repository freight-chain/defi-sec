...
const CHECKER_ERRORS = {
    ...
    DEFI-SEC_ID_FORMAT: {
        code: '101',
        success: 'Valid DEFI-SEC-ID format'
    },
    ISSUE_COUNT: {
        code: '102',
...
const GITHUB_CHECKERS = {
    ...
    issues: {
        DEFI-SECID: (id) => /^(DEFI-SEC-)\d{3}$/.test(id),
...
const issueValidator = (config, content, issue) => {
    if (!GITHUB_CHECKERS.issues.DEFI-SECID(issue.id)) {
        logError(config, 101, 'Wrong DEFI-SEC-ID format!');
    }
